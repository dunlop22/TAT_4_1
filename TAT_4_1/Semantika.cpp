...
Tree* Tree::SemInclude(LEX a, OBJ_TYPE ot, DATA_TYPE t)
{
	if (DupControl(cur, a))
	{
		Tree* root = FindRoot();
		root->Print();
		scan->PrintError("Ïîâòîğíîå îïèñàíèå èäåíòèôèêàòîğà", a, '\0');
	}
...
Tree* Tree::SemGetVar(LEX a)
{
	...
	if (v == NULL)
	{
		Tree* root = FindRoot();
		root->Print();
		scan->PrintError("Îòñóòñòâóåò îïèñàíèå èäåíòèôèêàòîğà", a, '\0');
	}
	...
	if (v->node->objType == ObjFunct)
	{
		Tree* root = FindRoot();
		root->Print();
		scan->PrintError("Íåâåğíîå èñïîëüçîâàíèå èìåíè ôóíêöèè", a, '\0');
	}
	...
	if (v->node->objType == ObjClass)
	{
		Tree* root = FindRoot();
		root->Print();
		scan->PrintError("Íåâåğíîå èñïîëüçîâàíèå èìåíè êëàññà", a, '\0');
	}
...
Tree* Tree::SemGetFunct(LEX a)
{
	...
	if (v == NULL)
	{
		Tree* root = FindRoot();
		root->Print();
		scan->PrintError("Îòñóòñòâóåò îïèñàíèå ôóíêöèè", a, '\0');
	}
	...
	if (v->node->objType != ObjFunct)
	{
		Tree* root = FindRoot();
		root->Print();
		scan->PrintError("Èäåíòèôèêàòîğ íå ÿâëÿåòñÿ èìåíåì ôóíêöèè", a, '\0');
	}
...
Tree* Tree::SemGetClass(LEX a)
{
	...
	if (v == NULL)
	{
		Tree* root = FindRoot();
		root->Print();
		scan->PrintError("Îòñóòñòâóåò îïèñàíèå êëàññà", a, '\0');
	}
	...
	if (v->node->objType != ObjClass)
	{
		Tree* root = FindRoot();
		root->Print();
		scan->PrintError("Èäåíòèôèêàòîğ íå ÿâëÿåòñÿ èìåíåì êëàññà", a, '\0');
	}
